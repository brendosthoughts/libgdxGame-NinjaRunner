...
public class GameOverScreen implements Screen{
	...
	public GameOverScreen (Game game, int level,Vector2 revive ) {
		this.game = game;
		this.levelNum=level;
		...
		retryBounds= new Rectangle(CAMERA_WIDTH/4, 0, CAMERA_WIDTH/4, 2*CAMERA_HEIGHT/5);
		continueBounds = new Rectangle(CAMERA_WIDTH/3, 2*CAMERA_HEIGHT/5, CAMERA_WIDTH/3, 2*CAMERA_HEIGHT/5 );
		mainMenuBounds = new Rectangle(CAMERA_WIDTH/2, 0, CAMERA_WIDTH/4, 2*CAMERA_HEIGHT/5);
	...
	public void update () {
		if (Gdx.input.justTouched()) {
			...
			if(continueBounds.contains(touchPoint.x, touchPoint.y) && revivePos!=null) {
				Assets.playSound(Assets.clickSound);
				game.setScreen(new GameScreen(game, levelNum, revivePos));
